...
public class ClippingDemo2 extends Application {
	...
	private Image teneriffa;
	private ImageView blurImV;
	...
	public void start(Stage primaryStage) {
		...
		teneriffa = new Image(getClass().getResourceAsStream("teneriffa.jpg"));
		imageView = new ImageView(teneriffa);
		blurImV = new ImageView(teneriffa);

		imageStack = new StackPane(imageView);
		root.getChildren().addAll(imageStack, buildClippingChoiceBox());
	...
	private Node buildClippingChoiceBox() {
		...
		whitePicture.setOnAction(e -> imageView.setClip(whiteImagePane));
		...
		blur.setOnAction(e -> {
			...
			blurImV.setClip(rec2);
			imageStack.getChildren().add(blurImV);
